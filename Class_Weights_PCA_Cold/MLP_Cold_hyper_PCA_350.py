...
def data():
    ...
    pca = PCA(n_components=350).fit(train_data_input)
    ...
    import pickle     
    pickle.dump( pca, open( "best_pca350.pickle", "wb" ) )
    label_count={}
...
def model(train_input, train_output, validation_input, validation_output, test_input, test_output, cweights):
    ...
    model.add(Dense({{choice([150,300,500,750,1000])}}, input_shape=(350,), init={{choice(['glorot_normal','glorot_uniform'])}}))
...
best_model.save('best_model350.h5')

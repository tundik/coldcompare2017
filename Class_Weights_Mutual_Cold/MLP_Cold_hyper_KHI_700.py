...
def data():
    ...
    kbest = SelectKBest(score_func=mutual_info_classif, k=700).fit(train_data_input, train_output[:,0])
    ...
    import pickle     
    pickle.dump( kbest, open( "best_mut700.pickle", "wb" ) )
    label_count={}
...
def model(train_input, train_output, validation_input, validation_output, test_input, test_output, cweights):
    ...
    model.add(Dense({{choice([150,300,500,750,1000])}}, input_shape=(700,), init={{choice(['glorot_normal','glorot_uniform'])}}))
...
best_model.save('best_model700.h5')

...
with h5py.File('ds_430_fullheight.h5', 'w') as hf:
...
earlyStopping = EarlyStopping(monitor='val_recall', patience=15, mode='max')
checkpointer = ModelCheckpoint(filepath=bestWeightsPath, monitor='val_recall', verbose=1, save_best_only=True, mode='max')
...
fitting_result=model.fit(X_train, y_train, nb_epoch=100, batch_size=32, callbacks=[recallCallback, earlyStopping, checkpointer], validation_data=(X_devel,y_devel), class_weight = cweights)

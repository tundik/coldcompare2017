...
merged_layers = merge([model_cnn.layers[-3].output, model_dnn2.layers[-3].output], mode='concat')
...
model = Model(input=[model_cnn.input, model_dnn2.input], output=output1)
...
earlyStopping = EarlyStopping(monitor='val_recall', patience=20, mode='max')

# ModelCheckpointer to save the best weights
from keras.callbacks import ModelCheckpoint
bestWeightsFilePath = "best_weights_ensemble.hdf5"
checkpointer = ModelCheckpoint(filepath=bestWeightsFilePath, verbose=1, monitor="val_recall", save_best_only=True, mode='max')

model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy']) #best: adadelta
'''
extraSamplesFromDevel = int(len(X_devel_cnn)/4.0)
...
'''
model.fit([X_train_cnn, X_train_dnn2], y_train_cnn, nb_epoch=1000, batch_size=64,
          callbacks=[recallCallback, earlyStopping, checkpointer],
          validation_data=([X_devel_cnn, X_devel_dnn2], y_devel_cnn), class_weight=cweights)


print("Loading weights with smallest loss")
model.load_weights(bestWeightsFilePath)

predicted_labels = lb.inverse_transform(model.predict([X_devel_cnn, X_devel_dnn2]))

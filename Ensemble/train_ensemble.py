...
x = Dense(1024, activation = 'relu', init='glorot_normal')(merged_layers)
x = Dropout(0.5)(x)
x = Dense(1024, activation = 'relu', init='glorot_normal')(x)
x = Dropout(0.5)(x)
...
earlyStopping = EarlyStopping(monitor=monitor, patience=20, mode=mode)
...
model.compile(loss='categorical_crossentropy', optimizer='nadam',  metrics=['accuracy']) #best: adadelta
...
model.fit([X_train_cnn, X_train_dnn1, X_train_dnn2, X_train_boaw], y_train_cnn, nb_epoch=1000, batch_size=100,
...
'''
model.fit([X_train_cnn, X_train_dnn1, X_train_dnn2], y_train_cnn, nb_epoch=1000, batch_size=64,
...
model.save("models/ensemble_specfull_pca450_mut250_boaw6299-acc.h5".format(recall))